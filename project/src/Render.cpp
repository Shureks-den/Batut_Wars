...
Render::Render(sf::RenderWindow &window)
    ...
    for (size_t i = 0; i <= 4 * 10; ++i) {  // TODO(ANDY) размерность карты
        ...
        _animation_layers[static_cast<size_t>(animation::LayerNom::EFFECTS)].push_back(std::move(portal));
    }
    for (size_t i = 0; i <= 4 * 10; ++i) {  // TODO(ANDY) размерность карты
        auto portal = _creator.get_animation(animation::Id::PORTAL);
    ...
    for (size_t i = 0; i <= x_count; ++i) {  // TODO(ANDY) размерность карты
        for (size_t j = 0; j <= y_count; ++j) {
            ...
            _animation_layers[static_cast<size_t>(animation::LayerNom::BACKGROUND)].push_back(std::move(space));
        }
    }
}
