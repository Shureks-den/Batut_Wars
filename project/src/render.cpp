...
void Render::update(sf::Time dt) {
  _view.setCenter(
      _status[0].position);  // TODO(ANDY) вместо 0 - id данного игрока
  for (auto &it : _animation_layers) {
    it.update(dt);
  }
}
...
void Render::set_status(const std::vector<Status> &status) {
  for (size_t i = 0; i < _status.size();
       ++i) {  // TODO(ANDY) Ломается при удалении. Переписать на list
...
void Render::add_animation(size_t lay, Status &status) {
  animation::Manager added(_holder.get_texture(status.animation_id),
                           status.position, status.angle);
