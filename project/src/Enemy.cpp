...
void Enemy::update(sf::Time dt) {
  ...
  if (!_is_player_spotted) {  // афк действия, если игрока нет рядом, просто
    _aimed = false;           // двигается, крутится
    if (_rotate_time > sf::seconds(1) &&
        _rotate_time <
            sf::seconds(7)) {  // простое перемещение запустите прочекайте
  ...
  if (_is_player_spotted) {  // TODO(anyone) сделать изящное торможение и/или
                             // другое перемещение
    give_acceleration(Direction::FORWARD);
    turn_to_player();
  }
...
void Enemy::turn_to_player() {  // проверить работоспособность точно запихнуть в
                                // update
  engine::Vector new_orientaion(_player_location.x - this->get_position().x,
                                _player_location.y - this->get_position().y);

  float rotate_angle =
      acos((new_orientaion.get_x() * this->get_orientation().get_x() +
            new_orientaion.get_y() * this->get_orientation().get_y()) /
           (new_orientaion.get_abs() * this->get_orientation().get_abs()));
  if (new_orientaion.get_y() * get_orientation().get_x() <
      get_orientation().get_y() * new_orientaion.get_x()) {
    rotate_angle *= -1;
  }
