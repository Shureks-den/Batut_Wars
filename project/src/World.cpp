...
void World::update(sf::Time d_time) {
  ...
  for (size_t i = 0; i < _players.size(); ++i) {
    sf::Vector2f position =
        _players[i]->get_position();  // TODO(ANDY) обновление status (после
  ...
  for (size_t i = 0; i < _bullet.size(); ++i) {
    sf::Vector2f position =
        _bullet[i]->get_position();  // TODO(ANDY) обновление status (после
                                     // реализации angle в Entity)
...
//     Status status =
//     to_status(dynamic_cast<engine::Entity&>(*_objects.back()));
...
std::queue<Player::Action> &World::get_actions() { return _actions; }
