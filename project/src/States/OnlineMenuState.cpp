...
OnlineMenuState::OnlineMenuState(StateStack& stack, Context context)
  ...
  ip_textbox =
      std::make_shared<GUI::Textbox>(*context.fonts, *context.textures);
  ...
  // TODO(ANDY) по нажанию - запуск сервера в отдельном потоке, вывод ip и port
  // в текстбоксы, запись ip и port в context.network_info (мб бесполезное поле
  // - убрать) запуск server в context.server_thread context.client->connetc(ip,
  // port) Скорее всего, отдельный стейт на ожидние подключений
  ...
  back_button->set_callback([this]() { requestStackPop(); });
...
void OnlineMenuState::start_client() {
  getContext().network_info->first =
      ip_textbox->get_text();  // TODO(ANDY) ловить исключения
  getContext().network_info->second =
      static_cast<uint16_t>(std::stoi(port_textbox->get_text()));
  if (getContext().client->connect(*getContext().network_info)) {
    *getContext().client_thread =
        std::thread([this]() { getContext().client->run(); });
...
void OnlineMenuState::start_server() {
  *getContext().server_thread =
      std::thread([this]() { getContext().server->run(); });
