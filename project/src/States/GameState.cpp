...
GameState::GameState(StateStack& stack, Context context)
    : GameStateBase(stack, context), _world() {
  // ИГРОК
  context.music->play(Music::MissionTheme);
  auto main_ship = std::unique_ptr<engine::MoveAble>(
      new space::Ally);  // TODO(ANDY) инициализация карты
  ...
  // ЧЕРНАЯ ДЫРОЧКА
  auto blackhole =
      std::unique_ptr<space::Massive>(new space::Massive(100, 125.0f));
  ...
  // ХОЛОДНОЕ СЕРДЦЕ
  auto ice_planet_1 =
      std::unique_ptr<space::IcePlanet>(new space::IcePlanet(125.0f));
  ...
  // КОМЕТЫ
  auto comet_1 =
      std::unique_ptr<space::Comet>(new space::Comet(engine::Vector(50, 50)));
  comet_1->set_position(sf::Vector2f(2200, 1000));
  _world.push_back(std::move(comet_1));
  auto comet_2 =
      std::unique_ptr<space::Comet>(new space::Comet(engine::Vector(70, 70)));
  comet_2->set_position(sf::Vector2f(2300, 1000));
  _world.push_back(std::move(comet_2));
  auto comet_3 =
      std::unique_ptr<space::Comet>(new space::Comet(engine::Vector(100, 100)));
  ...
  auto comet_5 =
      std::unique_ptr<space::Comet>(new space::Comet(engine::Vector(80, 0)));
  comet_5->set_position(sf::Vector2f(10, 100));
  _world.push_back(std::move(comet_5));
  auto comet_6 =
      std::unique_ptr<space::Comet>(new space::Comet(engine::Vector(0, 100)));
...
bool GameState::update(sf::Time dt) {
  _world.update(dt);
  _render.set_status(
      _world.get_status());  // Обмен данными между _render и _world
  _render.update(dt);
  update_statistic(dt);
