...
Game::Game()
    : _window(sf::VideoMode(640, 480), "Input", sf::Style::Close),
      _render(_window),
      _frames(0) {
...
void Game::update_statistic(sf::Time time) {
  ...
  if (_update_time >= sf::seconds(1.0f)) {
    _statistic_text.setString(
        ...
        "Time / Update = " +
        std::to_string(_update_time.asMicroseconds() / _frames) + "us");
...
void Game::render(sf::Time dt) {
  ...
  _render.draw();
  _window.setView(
      _render.get_view());  // Вид устанавливается в _render.set_status
