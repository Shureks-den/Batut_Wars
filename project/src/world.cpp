...
static Status to_status(engine::Entity &entity) {
  ...
  status.lay_id = static_cast<size_t>(
      animation::LayerNom::OBJECTS);  // TODO(ANDY) получить LayerNom по Id
...
void World::update(sf::Time d_time) {
  ...
  for (size_t i = 0; i < _objects.size(); ++i) {
    ...
    _status[i].position =
        _objects[i]->get_position();  // TODO(ANDY) обновление status (после
...
void World::do_action(
  ...
  switch (action) {
    ...
    case Action::MOVE_LEFT:
      ship->rotate(engine::to_radian(-60.0) *
                   d_time.asSeconds());  // TODO(ANDY) довести до ума
      _status[id].angle -= 60.0 * d_time.asSeconds();
      break;
    case Action::MOVE_RIGHT:
      ship->rotate(engine::to_radian(60.0) *
                   d_time.asSeconds());  // TODO(ANDY) довести до ума блять
      _status[id].angle += 60.0 * d_time.asSeconds();
      break;
...
std::vector<Status> World::get_status() { return _status; }

std::queue<Action> &World::get_actions() { return _actions; }
