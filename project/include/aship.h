...
Ship::Ship(sf::Vector2f position, float angle)
    : Manager(Id::SHIP, position, angle) {
  set_switch_time(sf::seconds(0.1f));
  set_image_count(sf::Vector2u(5, 1));

  sf::Vector2f size(_body.getTextureRect().width,
                    _body.getTextureRect().height);
  size.x /= 2;
  size.y /= 2;
  _body.setSize(size);
  size.x /= 2;
...
Space::Space(sf::Vector2f position, float angle)
    : Manager(Id::SPACE, position, angle) {
