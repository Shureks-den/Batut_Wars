...
engine::Vector::Vector(double x, double y) {
	this->_x = x;
	this->_y = y;
}
...
double engine::Vector::set_x(double x) {
	this->_x = x;
	return this->_x;
}

double engine::Vector::set_y(double y) {
	this->_y = y;
	return this->_y;
}
...
void engine::Vector::rotate(double angle) {
	this->_x = this->_x * cos(angle) - this->_y * sin(angle);
...
float engine::SpaceObject::get_x() const {
	return this->_coordinate.x;
}

float engine::SpaceObject::get_y() const {
	return this->_coordinate.y;
}

float engine::SpaceObject::set_x(float x) {
	this->_coordinate.x = x;
	return this->_coordinate.x;
}

float engine::SpaceObject::set_y(float y) {
...
namespace engine {
	...
	Vector MoveAble::d_acceleration(Direction direction, float speed_abs, float d_acceleration) {
		Vector delta(_speed.get_x() / speed_abs, _speed.get_y() / speed_abs);
	...
	void MoveAble::give_acceleration(Direction direction, float d_acceleration) {
		...
		Vector delta(this->d_acceleration(direction, speed_abs, d_acceleration));
		_acceleration += delta;
	}
}
